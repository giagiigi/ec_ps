 ...
 */
public class FavoriteListMoveAction extends WebFrontAction<FavoriteListBean> {
  ...
  public WebActionResult callService() {
    ...
    if (nextParam.equals("shop")) {
      ...
      if (shop == null) {
        addErrorMessage(WebMessage.get(ServiceErrorMessage.NO_DATA_ERROR, Messages.getString("web.action.front.mypage.FavoriteListMoveAction.0")));
      ...
      if (!catalogSv.isListed(shopCode, commodityCode)) {
        addErrorMessage(WebMessage.get(CatalogErrorMessage.NO_COMMODITY_DETAIL_ERROR));
        setRequestBean(getBean());

        return FrontActionResult.RESULT_SUCCESS;
      }

      String skuCode = getRequestParameter().get("selectedSkuCode");
      setNextUrl("/app/catalog/detail/init/" + commodityCode + "?selectedSkuCode=" + skuCode);
    ...
    // 前画面情報設定
    DisplayTransition.add(null, "/app/mypage/favorite_list/init", getSessionContainer());
