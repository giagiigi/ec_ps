 ...
 */
public class MailMagazineCancelAction extends MailMagazineBaseAction {
  ...
  public WebActionResult callService() {
    ...
    String dencrypt = PasswordUtil.decrypt(encrypt); // 複合
    ...
    if (element.length == 3) {
      ...
      if (!DateUtil.isPeriodDate(requestedDatetime, closedDatetime, sysdate)) {
        addErrorMessage(WebMessage.get(MypageErrorMessage.EXPIRED_URL_ERROR,  Messages
            .getString("web.action.front.customer.MailMagazineCancelAction.0")));
      ...
      // 登録解除に成功したメルマガのリスト
      List<String> canceledMailMagazineTitleList = new ArrayList<String>();
      // 登録されていないメルマガのリスト
      List<String> notRegisterMailMagazineTitleList = new ArrayList<String>();
      // 既に消されていたメルマガのリスト
      ...
      for (int i = 0; i < mailMagazineCodeList.length; i++) {
        ...
        result = svc.cancelMailMagazineSubscriber(subscriber);

        if (result.hasError()) {
          for (ServiceErrorContent error : result.getServiceErrorList()) {
            if (error.equals(CommonServiceErrorContent.NO_DATA_ERROR)) {
              logger.error("not canceled. mailMagazineCode : " + mailMagazineCodeList[i]);
              notRegisterMailMagazineTitleList.add(svc.getMailMagazine(mailMagazineCodeList[i]).getMailMagazineTitle());
            }
          }
        } else {
          canceledMailMagazineTitleList.add(svc.getMailMagazine(mailMagazineCodeList[i]).getMailMagazineTitle());
        }
      }

      // メッセージ生成
      if (notRegisterMailMagazineTitleList.size() > 0) {
        addErrorMessage(WebMessage.get(MypageErrorMessage.MAILMAGAZINE_CANCELED_ERROR));
        for (String title : notRegisterMailMagazineTitleList) {
      ...
      if (deletedMailMagazineTitleList.size() > 0) {
        addErrorMessage(WebMessage.get(ServiceErrorMessage.NO_DATA_ERROR, Messages
            .getString("web.action.front.customer.MailMagazineCancelAction.2")));
        for (String s : deletedMailMagazineTitleList) {
          addErrorMessage("・" + s);
        }
      }
      if (canceledMailMagazineTitleList.size() > 0) {
        addInformationMessage(WebMessage.get(MypageDisplayMessage.MAILMAGAZINE_CANCEL_COMPLETE));
        for (String s : canceledMailMagazineTitleList) {
