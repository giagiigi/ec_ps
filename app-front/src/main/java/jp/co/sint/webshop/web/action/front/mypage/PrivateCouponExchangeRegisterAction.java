 ...
 */
public class PrivateCouponExchangeRegisterAction extends WebFrontAction<PrivateCouponExchangeBean> {
  ...
  public WebActionResult callService() {
    ...
    PrivateCouponExchangeBean bean = getBean();
    ...
    if (ncRule != null) {
      if (goodPoint >= ncRule.getExchangePointAmount()) {
        ...
        newCouponHistoryDto.setUseStartDatetime(DateUtil.addDate(DateUtil.getSysdate(), Integer.parseInt(NumUtil.toString(startNum))));
        
        Date endDatetimeTemp = DateUtil.addDate(newCouponHistoryDto.getUseStartDatetime(), Integer.parseInt(NumUtil.toString(endNum - 1L)));
...
//    // メールを送信する
//    MailingService sendMail = ServiceLocator.getMailingService(getLoginInfo());
//    sendMail.sendFriendCouponMail(getLoginInfo().getCustomerCode());
    setRequestBean(bean);

    // 履歴登録完了
    setNextUrl("/app/mypage/private_coupon_exchange/init/" + WebConstantCode.COMPLETE_INSERT + "/" + ncRule.getCouponCode());
