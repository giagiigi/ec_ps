...
import jp.co.sint.webshop.service.tmall.TmallSearchOrderDownloadNumService;
...
public class TmallSearchOrderDownloadNums implements TmallSearchOrderDownloadNumService {
  ...
  public List<String> searchOrderDownLoadNums(String startTime, String endTime) {
    ...
    String[] statusStr = {
        "WAIT_BUYER_PAY", "WAIT_SELLER_SEND_GOODS", "TRADE_FINISHED", "TRADE_CLOSED", "TRADE_CLOSED_BY_TAOBAO",
        "TRADE_NO_CREATE_PAY"
    };
    ...
    if (mailFlg) {
      ...
      sb.append("以下为获取今日订单数失败警告:<BR><BR>");
      sb.append("由于网络通信失败，获取今日订单数API无法调用。<BR><BR>");
      sb.append("下载失败时间段:" + startTime + "~" + endTime);
      mailInfo.setText(sb.toString());
      mailInfo.setSubject("获取今日订单数失败警告");
