 ...
 * @author kousen
 */
public class AlipayResponseNotify extends HttpServlet {
  ...
  protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
    ...
    logger.info(Messages.getString("jp.co.sint.webshop.ext.alipay.AlipayResponseNotify.0")+"ORDER_NO:  " + order_no);
    ...
    // 订单存在检查
    logger.info(Messages.getString("jp.co.sint.webshop.ext.alipay.AlipayResponseNotify.1")+ trade_status);
    if (container == null || container.getOrderHeader() == null) {
      out.println("fail");
      out.flush();
      out.close();
      logger.error(Messages.getString("jp.co.sint.webshop.ext.alipay.AlipayResponseNotify.2")+"fail:  " + order_no);
      return;
    }
    ...
    if (trade_status.equals("TRADE_SUCCESS") && mysign.equals(sign) && responseTxt.equals("true")) {
      if (!PaymentStatus.PAID.longValue().equals(header.getPaymentStatus())) {
        ...
        if (svcResult.hasError()) {
          result = false;
          logger.error(Messages.getString("jp.co.sint.webshop.ext.alipay.AlipayResponseNotify.3") + order_no);
        } else {
          logger.info(Messages.getString("jp.co.sint.webshop.ext.alipay.AlipayResponseNotify.4") + order_no);
          out.flush();
          out.close();
          return;
        }
    ...
    if (result) {
      ...
      out.flush();
      out.close();
      logger.error(Messages.getString("jp.co.sint.webshop.ext.alipay.AlipayResponseNotify.5")+"OrderNo:  " + order_no);
    }
  }
