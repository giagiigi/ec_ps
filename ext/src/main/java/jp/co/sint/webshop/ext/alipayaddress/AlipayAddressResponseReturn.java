...
public class AlipayAddressResponseReturn extends HttpServlet {
   ...
   */
    ...
    String receiveMobile = request.getParameter("receive_mobile");
		CustomerService customerService = ServiceLocator
				.getCustomerService(ServiceLoginInfo.getInstance());
    ...
    customer = customerService.getCustomerCode(tmall_user_id, CustomerStatus.WITHDRAWED.getValue());
		 if (svcResult.hasError()) {
	      ...
	      return;
		   ...
		   request.getRequestDispatcher("/app/customer/fastpay/init/" +customer.getCustomerCode()).forward(request, response);
		   return;
		 }
	}
