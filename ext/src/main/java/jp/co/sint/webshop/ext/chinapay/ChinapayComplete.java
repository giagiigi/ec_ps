 ...
 *
 */
public class ChinapayComplete extends HttpServlet {
  ...
  protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
    ...
    try {
      ...
      // 订单编号判断
      result = validateItems(bean, "private1");
      if (!result) {
        req.getRequestDispatcher("/app/common/index").forward(req, resp);
        out.println("NG");
        out.flush();
        out.close();
        logger.error("ChinaPay: Update Payment Error!(private1:" + bean.getPrivate1() + ")");
        return;
      }
      // 银联商户信息初期化
      String type = "";
      ...
      if (result && bean.getTransactionStatus().equals("1001")) {
        ...
        // 20110512 shiseidou update start
        // req.getRequestDispatcher("/app/order/complete").forward(req, resp);
      if (result) {
        req.getRequestDispatcher("/app/order/external_payment/complete/" + orderNo).forward(req, resp);
      } else {
        req.getRequestDispatcher("/app/common/index").forward(req, resp);
      }
      // 20110512 shiseidou update end
      if (!result) {
  ...
  private boolean validateItems(Object objBean, String... fieldName) {
    List<ValidationResult> resultList = BeanValidator.partialValidate(objBean, fieldName).getErrors();
