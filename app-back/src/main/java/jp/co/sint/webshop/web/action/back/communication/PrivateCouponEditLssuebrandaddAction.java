 ...
 * @author OB.
 */
public class PrivateCouponEditLssuebrandaddAction extends PrivateCouponEditBaseAction {
  ...
  public WebActionResult callService() {
    ...
    String[] codeStringArray = bean.getLssueBrandCode().split("\r\n");
    ...
    if (codeStringArray.length > 0 && StringUtil.hasValue(codeStringArray[0])) {
      ...
      for (int i = 0; i < codeStringArray.length; i++) {
        if (StringUtil.hasValue(codeStringArray[i])) {
          if (!brandDao.exists(getLoginInfo().getShopCode(), codeStringArray[i])) {
            addErrorMessage(WebMessage.get(ServiceErrorMessage.NO_DATA_ERROR, MessageFormat.format("发行关联品牌编号：{0}",
                codeStringArray[i])));
            brandExit = false;
          }
        }
      ...
      for (int i = 0; i < codeStringArray.length; i++) {
        if (StringUtil.hasValue(codeStringArray[i])) {
          ...
          newCouponRuleLssueInfo.setBrandCode(codeStringArray[i]);
          ...
          // 登录顾客别优惠券发行规则
          serviceResult = svc.insertNewCouponRuleLssue(newCouponRuleLssueInfo, WebConstantCode.LSSUE_BRAND_COMPLETE_REGISTER);
          ...
          if (serviceResult.hasError()) {
            for (ServiceErrorContent error : serviceResult.getServiceErrorList()) {
              if (error.equals(CommonServiceErrorContent.VALIDATION_ERROR)) {
                ...
                // 数据重复错误信息
                addErrorMessage(WebMessage.get(ServiceErrorMessage.REGISTER_ERROR, MessageFormat.format("发行关联品牌编号：{0}",
                    codeStringArray[i])));
                setRequestBean(bean);
                return BackActionResult.RESULT_SUCCESS;
              }
    ...
    setNextUrl("/app/communication/private_coupon_edit/init/edit/" + bean.getCouponCode() + "/"
        + WebConstantCode.LSSUE_BRAND_COMPLETE_REGISTER);
  ...
  public boolean validate() {
    PrivateCouponEditBean bean = getBean();
    if (!StringUtil.hasValue(bean.getLssueBrandCode())) {
      List<String> messages = new ArrayList<String>();
      messages.add("发行关联品牌编号必须输入！");
