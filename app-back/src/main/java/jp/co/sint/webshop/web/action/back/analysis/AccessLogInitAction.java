...
public class AccessLogInitAction extends WebBackAction<AccessLogBean> {
  ...
  public WebActionResult callService() {
    ...
    bean.setSearchYear(DateUtil.getYYYY(DateUtil.getSysdate()));
    bean.setSearchMonth(DateUtil.getMM(DateUtil.getSysdate()));
    bean.setSearchDay(DateUtil.getDD(DateUtil.getSysdate()));
    // 分析データ出力_サイトの権限を持つユーザのみがCSV出力可能
    bean.setExportAuthority(Permission.ANALYSIS_DATA_SITE.isGranted(getLoginInfo()));
    bean.setDisplayMode("day");
    ...
    clientList.add(new NameValue(Messages.getString(
        "web.action.back.analysis.AccessLogInitAction.0"), ""));
  ...
  public String getActionName() {
    return Messages.getString("web.action.back.analysis.AccessLogInitAction.1");
  }
