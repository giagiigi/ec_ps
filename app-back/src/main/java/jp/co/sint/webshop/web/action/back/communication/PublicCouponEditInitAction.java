 ...
 * @author OB
 */
public class PublicCouponEditInitAction extends PublicCouponEditBaseAction {
    ...
    CommunicationService communicationService = ServiceLocator.getCommunicationService(getLoginInfo());
	  // 更新或注册的情况下,显示信息
		if (getRequestParameter().getPathArgs().length == 2) {
			...
			getBean().setCouponTypeName(CouponType.COMMON_DISTRIBUTION.getName());
			...
			if(getRequestParameter().getPathArgs().length==3 && !StringUtil.isNullOrEmpty(getRequestParameter().getPathArgs()[2]) && getRequestParameter().getPathArgs()[2].equals("succeed")){
			  newCouponRule = communicationService.getPrivateCoupon(getBean().getCouponId());
	...
	public boolean authorize() {
		...
		if (super.authorize()) {
			...
			if(getRequestParameter().getPathArgs().length == 0 
					&& Permission.PUBLIC_COUPON_READ_SHOP.isGranted(getLoginInfo()) == true
