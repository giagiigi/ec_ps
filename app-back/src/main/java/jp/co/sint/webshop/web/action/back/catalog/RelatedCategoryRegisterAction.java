 ...
 */
public class RelatedCategoryRegisterAction extends WebBackAction<RelatedCategoryBean> {
  ...
  public WebActionResult callService() {
    ...
    // ログインユーザがショップ、または一店舗版の場合はログイン情報からショップコードを取得
    if (getLoginInfo().isShop() || getConfig().isOne()) {
      reqBean.setSearchShopCode(getLoginInfo().getShopCode());
      reqBean.getEdit().setShopCode(getLoginInfo().getShopCode());
    }

    if (StringUtil.isNullOrEmpty(reqBean.getEdit().getShopCode())) {
      addErrorMessage(WebMessage.get(ServiceErrorMessage.NO_DATA_ERROR,
          Messages.getString("web.action.back.catalog.RelatedCategoryRegisterAction.0")));
    ...
    categoryCommodity.setShopCode(reqBean.getEdit().getShopCode());
    categoryCommodity.setCategoryCode(reqBean.getCategoryCode());
    categoryCommodity.setCategorySearchPath("/"); //導出項目なので設定不要。
    categoryCommodity.setCommodityCode(reqBean.getEdit().getCommodityCode());
    for (int i = 0; i < DIContainer.getWebshopConfig().getCategoryAttributeMaxCount(); i++) {
      if (StringUtil.hasValue(reqBean.getAttributeList().get(i).getCategoryAttributeName())) {
        ...
        categoryAttributeValue.setShopCode(reqBean.getEdit().getShopCode());
    ...
    ServiceResult result = service.registerCategoryCommodity(categoryCommodity, categoryAttributeValueList);
    ...
    if (result.hasError()) {
      for (ServiceErrorContent errorContent : result.getServiceErrorList()) {
        if (errorContent.equals(CommonServiceErrorContent.DUPLICATED_REGISTER_ERROR)) {
          addErrorMessage(WebMessage.get(ServiceErrorMessage.DUPLICATED_REGISTER_ERROR,
              Messages.getString("web.action.back.catalog.RelatedCategoryRegisterAction.1")));
        } else if (errorContent.equals(CommonServiceErrorContent.NO_DATA_ERROR)) {
          addErrorMessage(WebMessage.get(ServiceErrorMessage.NO_DATA_ERROR,
              Messages.getString("web.action.back.catalog.RelatedCategoryRegisterAction.2")));
      ...
      setNextUrl("/app/catalog/related_category/init/" + WebConstantCode.COMPLETE_INSERT);
    }
  ...
  public boolean validate() {
    RelatedCategoryBean relatedCategoryBean = getBean();
    List<Boolean> result = new ArrayList<Boolean>();
    result.add(validateBean(relatedCategoryBean));
  ...
  public String getActionName() {
    return Messages.getString("web.action.back.catalog.RelatedCategoryRegisterAction.4");
  }
