 ...
 * @author OB.
 */
public class PrivateCouponEditImportaddAction extends PrivateCouponEditBaseAction {
  ...
  public WebActionResult callService() {
    ...
    // 選択されたレビューIDを取得する
    String[] values = getRequestParameter().getAll("importCommodityType");
    CommunicationService svc = ServiceLocator.getCommunicationService(getLoginInfo());

    if (newCouponRuleUseInfo == null) {
      newCouponRuleUseInfo = new NewCouponRuleUseInfo();
    }
    if (values.length > 0 && values[0] != "") {
      ...
      svc.deletePrivateCouponUse(bean.getCouponCode(), "", WebConstantCode.IMPORT_COMPLETE_REGISTER);
      // 添加dto内容
      for (String importCode : values) {
        // 判断优惠券规则编号是否已经存在
        ...
        newCouponRuleUseInfo.setImportCommodityType(NumUtil.toLong(importCode));
    ...
    setNextUrl("/app/communication/private_coupon_edit/init/edit/" + bean.getCouponCode() + "/"
        + WebConstantCode.IMPORT_COMPLETE_REGISTER);
