 ...
 */
public class DeliveryTypeDeleteDeliveryAction extends WebBackAction<DeliveryTypeBean> {
  ...
  public boolean validate() {
    if (StringUtil.isNullOrEmpty(getBean().getDeleteDeliveryTypeNo())) {
      addErrorMessage(WebMessage.get(ShopErrorMessage.CODE_FAILED,
          Messages.getString("web.action.back.shop.DeliveryTypeDeleteDeliveryAction.0"))); //$NON-NLS-1$
      return false;
    }
    return true;
  }
  ...
  public WebActionResult callService() {
    ...
    ServiceResult result = service.deleteDeliveryType(getLoginInfo().getShopCode(), NumUtil.toLong(getBean()
        .getDeleteDeliveryTypeNo()));
    ...
    if (result.hasError()) {
      for (ServiceErrorContent error : result.getServiceErrorList()) {
        ...
        } else if (error.equals(ShopManagementServiceErrorContent.USED_DELIVERY_TYPE)) {
          setRequestBean(getBean());
          setNextUrl(null);
    ...
    setNextUrl("/app/shop/delivery_type/init/" + WebConstantCode.COMPLETE_DELETE);
  ...
  public String getActionName() {
    return Messages.getString("web.action.back.shop.DeliveryTypeDeleteDeliveryAction.1");
  }
