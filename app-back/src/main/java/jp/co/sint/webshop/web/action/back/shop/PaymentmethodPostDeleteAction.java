 ...
 */
public class PaymentmethodPostDeleteAction extends WebBackAction<PaymentmethodPostBean> {
  ...
  public WebActionResult callService() {
    ...
    ServiceResult result = service.deletePost(shopCode, NumUtil.toLong(bean.getPaymentMethodNo()),bean.getDeletePost().getPostAccountNo());
    //deletePost()参数  Long paymentMethodNo,String postAccountNo
    if (result.hasError()) {
      for (ServiceErrorContent error : result.getServiceErrorList()) {
        if (error.equals(ShopManagementServiceErrorContent.DELETE_ALL_POST)) {
          addErrorMessage(WebMessage.get(ShopErrorMessage.DELETE_ALL_POST));
      ...
      setRequestBean(bean);
      setNextUrl("/app/shop/paymentmethod_post/init/" + bean.getShopCode() + "/" + bean.getPaymentMethodNo() + "/delete");
    }
    return BackActionResult.RESULT_SUCCESS;
  }
  ...
  public String getActionName() {
    return Messages.getString("web.action.back.shop.PaymentmethodPostDeleteAction.0");
  }
