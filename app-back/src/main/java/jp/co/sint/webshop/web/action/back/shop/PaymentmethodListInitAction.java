...
import jp.co.sint.webshop.service.shop.PaymentmethodListSearchCondition;
 ...
 */
public class PaymentmethodListInitAction extends PaymentMethodListBaseAction {
  ...
  public WebActionResult callService() {
    // ログインユーザよりショップコードを取得
    PaymentmethodListSearchCondition searchCondition = new PaymentmethodListSearchCondition();
    searchCondition.setShopCode(getLoginInfo().getShopCode());
    if (Permission.SHOP_MANAGEMENT_READ_SITE.isGranted(getLoginInfo())) {
      searchCondition.setSitePermission(true);
    }
    // ログインユーザの支払方法を全件取得
    List<PaymentMethodDetail> paymentMethodList = getPaymentMethodDetailList(getLoginInfo().getShopCode());

    PaymentmethodListBean nextBean = new PaymentmethodListBean();
    nextBean.setPaymentMethodList(paymentMethodList);
    setRequestBean(nextBean);

    if (getRequestParameter().getPathArgs().length > 0) {

      String information = "";
      if (getRequestParameter().getPathArgs()[0].equals(WebConstantCode.COMPLETE_DELETE + "_method")) {
        information = WebMessage.get(CompleteMessage.DELETE_COMPLETE,
            Messages.getString("web.action.back.shop.PaymentmethodListInitAction.0"));
      }
  ...
  public String getActionName() {
    return Messages.getString("web.action.back.shop.PaymentmethodListInitAction.1");
  }
