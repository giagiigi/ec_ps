 ...
 * @author cxw
 */
public class DeliveryCompanyEditMoveAction extends WebBackAction<DeliveryCompanyListBean> {
    ...
    if (params[params.length-1].equals("tmall")){
      TmallDeliveryRegion deliveryRegion = service.getDeliveryRegionTmall(getLoginInfo().getShopCode(), params[0], params[1]);
      ...
      DeliveryRegion deliveryRegion = service.getDeliveryRegion(getLoginInfo().getShopCode(), params[0], params[1]);
      if (deliveryRegion == null) {
        setNextUrl(null);
        setRequestBean(getBean());
        addErrorMessage(WebMessage.get(ServiceErrorMessage.NO_DATA_ERROR, "配送公司及地域"));
		...
		// 返回一览
		DisplayTransition.add(getBean(), "/app/shop/delivery_company_edit/init/" + params[0], getSessionContainer());
		
    // 配送希望日跳转处理
		if (params[params.length-1].equals("tmall")){
	    setNextUrl("/app/shop/delivery_company_date/init/" + params[0] + "/" + params[1] + "/tmall");
		} else {
	    setNextUrl("/app/shop/delivery_company_date/init/" + params[0] + "/" + params[1]);
		}
