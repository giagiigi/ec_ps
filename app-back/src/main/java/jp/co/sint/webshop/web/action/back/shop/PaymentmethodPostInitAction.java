...
/**
 * U1050530:郵便支払いのアクションクラスです

 * 
 * @author System Integrator Corp.
 */
public class PaymentmethodPostInitAction extends WebBackAction<PaymentmethodPostBean> {
  ...
  public WebActionResult callService() {
    ...
    ShopManagementService service = ServiceLocator.getShopManagementService(getLoginInfo());
    List<PostPayment> PostListResult = service.getPostList(shopCode, NumUtil.toLong(paymentMethodNo));

    // 取得した郵便支払いリストをbean用のリストに1件づつセット
    List<PostDetail> postList = new ArrayList<PostDetail>();
    for (PostPayment post : PostListResult) {
      PostDetail postDetail = new PostDetail();
      postDetail.setPostAccountName(post.getPostAccountName());
      postDetail.setPostAccountNo(post.getPostAccountNo());
      postDetail.setUpdatedDatetime(post.getUpdatedDatetime());
      postList.add(postDetail);
    }

    PaymentmethodPostBean bean = new PaymentmethodPostBean();
    ...
    if (completeInformation.equals(WebConstantCode.COMPLETE_REGISTER)) {
      addInformationMessage(WebMessage.get(CompleteMessage.REGISTER_COMPLETE,
          Messages.getString("web.action.back.shop.PaymentmethodPostInitAction.0")));
  ...
  public void prerender() {
    PaymentmethodPostBean bean = (PaymentmethodPostBean) getRequestBean();
  ...
  public String getActionName() {
    return Messages.getString("web.action.back.shop.PaymentmethodPostInitAction.1");
  }
