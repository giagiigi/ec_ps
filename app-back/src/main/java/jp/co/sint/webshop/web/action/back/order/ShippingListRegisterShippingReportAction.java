 ...
 */
public class ShippingListRegisterShippingReportAction extends ShippingListBaseAction {
  ...
  public boolean validate() {
    ...
    if (reqBean.getShippingCheck().isEmpty()) {
      addErrorMessage(WebMessage.get(ActionErrorMessage.NO_CHECKED,
          Messages.getString("web.action.back.orderShippingListRegisterShippingReportAction.0")));
      result = false;
    }

    if (!validateBean(reqBean.getReportEdit())) {
      result = false;
    }

    // 出荷日が未来日の場合エラーとする
    if (result && DateUtil.fromString(reqBean.getReportEdit().getShippingReportDate()).after(DateUtil.getSysdate())) {
    ...
    for (String shippingNo : reqBean.getShippingCheck()) {
      ...
      if (shippingHeader == null) {
        errorMessageDetailList.add(WebMessage.get(ServiceErrorMessage.NO_DATA_ERROR,
            MessageFormat.format(Messages.getString(
                "web.action.back.orderShippingListRegisterShippingReportAction.1"), shippingNo)));
        continue;
      }
      ...
      if (NumUtil.toString(shippingHeader.getShippingStatus()).equals(ShippingStatus.CANCELLED.getValue())) {
        ...
        errorMessageDetailList.add(WebMessage.get(OrderErrorMessage.CANCELLED_SHIPPING_WITH_NO, shippingHeader.getShippingNo(),
            Messages.getString("web.action.back.orderShippingListRegisterShippingReportAction.2")));
        continue;
      }
      ...
      shippedResult.setShippingDate(DateUtil.fromString(reqBean.getReportEdit().getShippingReportDate()));
      ...
      if (orderHeader == null) {
        errorMessageDetailList.add(WebMessage.get(ServiceErrorMessage.NO_DATA_ERROR,
            MessageFormat.format(Messages.getString(
                "web.action.back.orderShippingListRegisterShippingReportAction.3"), shippingHeader.getOrderNo())));
        continue;
      }
      ...
      if (!NumUtil.toString(shippingHeader.getShippingStatus()).equals(ShippingStatus.IN_PROCESSING.getValue())) {
        // 「出荷手配」の場合のみ出荷登録可能
        errorMessageDetailList.add(WebMessage.get(OrderErrorMessage.IMPOSSIBILITY_SHIPPING_REPORT, shippingHeader.getShippingNo()));
        continue;
      }

      if (shippedResult.getShippingDate().before(DateUtil.truncateDate(orderHeader.getOrderDatetime()))) {
        ...
        errorMessageDetailList.add(WebMessage.get(OrderErrorMessage.SHIPPING_INPUT_ORDERDATE_OR_LATER_WITH_NO, shippingHeader
            .getShippingNo(), Messages.getString("web.action.back.orderShippingListRegisterShippingReportAction.4")));
        continue;
      }

      if (shippedResult.getShippingDate().before(shippingHeader.getShippingDirectDate())) {
        // 出荷日が出荷指示日よりも前の日付の場合はエラー
        errorMessageDetailList.add(WebMessage.get(OrderErrorMessage.SHIPPING_DIRECT_INPUT_ORDERDATE_OR_LATER_WITH_NO,
            shippingHeader.getShippingNo()));
        continue;
      }

      serviceResult = service.registerShippingReport(shippedResult);//发送Email
      if (serviceResult.hasError()) {
        for (ServiceErrorContent errorContent : serviceResult.getServiceErrorList()) {
          if (errorContent.equals(CommonServiceErrorContent.NO_DATA_ERROR)) {
            errorMessageDetailList.add(WebMessage.get(ServiceErrorMessage.NO_DATA_ERROR,
                MessageFormat.format(Messages.getString(
                    "web.action.back.orderShippingListRegisterShippingReportAction.1"),
                    shippingHeader.getShippingNo())));
  ...
  public String getActionName() {
    return Messages.getString("web.action.back.orderShippingListRegisterShippingReportAction.5");
  }
