 ...
 */
public class DeliveryTypeEditUpdateAction extends WebBackAction<DeliveryTypeEditBean> {
  ...
  public WebActionResult callService() {
    ...
    deliveryType.setUpdatedDatetime(bean.getUpdateDatetime());
    ...
    for (RegionBlockCharge charge : bean.getRegionBlockChargeList()) {
      ...
      dto.setUpdatedDatetime(bean.getUpdateDatetime());
      chargeList.add(dto);
    }

    ServiceResult result = service.updateDeliveryType(deliveryType, chargeList);
    if (result.hasError()) {
      for (ServiceErrorContent content : result.getServiceErrorList()) {
        if (content == CommonServiceErrorContent.NO_DATA_ERROR) {
          addErrorMessage(WebMessage.get(ServiceErrorMessage.NO_DATA_ERROR,
              Messages.getString("web.action.back.shop.DeliveryTypeEditUpdateAction.0")));
          setRequestBean(bean);
    ...
    setNextUrl("/app/shop/delivery_type_edit/init/" + getBean().getDeliveryTypeNo() + "/" + WebConstantCode.COMPLETE_UPDATE);
    return BackActionResult.RESULT_SUCCESS;
  }
  ...
  public String getActionName() {
    return Messages.getString("web.action.back.shop.DeliveryTypeEditUpdateAction.1");
  }
