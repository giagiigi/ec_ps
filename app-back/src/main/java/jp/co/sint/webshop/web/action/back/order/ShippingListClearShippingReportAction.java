...
import jp.co.sint.webshop.data.domain.PaymentMethodType; // 10.1.4 10188 追加
import jp.co.sint.webshop.data.domain.PaymentStatus; // 10.1.4 10188 追加
 ...
 */
public class ShippingListClearShippingReportAction extends ShippingListBaseAction {
  ...
  public boolean validate() {
    ...
    if (reqBean.getShippingCheck().isEmpty()) {
      addErrorMessage(WebMessage.get(ActionErrorMessage.NO_CHECKED,
          Messages.getString("web.action.back.order.ShippingListClearShippingReportAction.0")));
      result = false;
    }
    ...
    for (String shippingNo : reqBean.getShippingCheck()) {
      ...
      if (shippingHeader == null) {
        errorMessageDetailList.add(WebMessage.get(ServiceErrorMessage.NO_DATA_ERROR,
            MessageFormat.format(Messages.getString("web.action.back.order.ShippingListClearShippingReportAction.1"),
                shippingNo)));
        continue;
      }
      ...
      if (ohDto == null) {
        errorMessageDetailList.add(WebMessage.get(ServiceErrorMessage.NO_DATA_ERROR,
            MessageFormat.format(Messages.getString("web.action.back.order.ShippingListClearShippingReportAction.2"),
      ...
      if (NumUtil.toString(shippingHeader.getShippingStatus()).equals(ShippingStatus.CANCELLED.getValue())) {
        ...
        errorMessageDetailList.add(WebMessage.get(OrderErrorMessage.CANCELLED_SHIPPING_WITH_NO, shippingHeader.getShippingNo(),
            Messages.getString("web.action.back.order.ShippingListClearShippingReportAction.3")));
        continue;
      }

      if (shippingHeader.getFixedSalesStatus().equals(FixedSalesStatus.FIXED.longValue())) {
        // 売上確定ステータスが「確定済み」の場合はステータスの変更不可
        errorMessageDetailList.add(WebMessage.get(OrderErrorMessage.FIXED_DATA_WITH_NO, shippingHeader.getShippingNo()));
        continue;
      }
      ...
      if (!shippingHeader.getShippingStatus().equals(ShippingStatus.SHIPPED.longValue())) {
        // 出荷ステータスが「出荷済」の場合以外は、ステータスの変更不可
        errorMessageDetailList.add(WebMessage.get(OrderErrorMessage.NO_STATUS_SHIPPED, shippingHeader.getShippingNo()));
        continue;
      }

      // 10.1.4 10188 修正 ここから
      if (ohDto.getPaymentMethodType().equals(PaymentMethodType.CREDITCARD.getValue())
          && ohDto.getPaymentStatus().equals(PaymentStatus.PAID.longValue())) {
        // 「クレジットカード払い」かつ「入金済み」の場合は売上送信済みとみなし出荷取消不可
        errorMessageDetailList.add(WebMessage.get(OrderErrorMessage.CLEAR_SHIPPING_FAILED_FOR_TRANSPORTED, shippingHeader
            .getShippingNo()));
        continue;
      }
      ...
      shippingHeader.setShippingDirectDate(DateUtil.fromString(reqBean.getEdit().getInputShippingDate()));
      shippingHeader.setShippingStatus(NumUtil.toLong(ShippingStatus.IN_PROCESSING.getValue()));
      ...
      report.setUpdatedUser(getLoginInfo().getLoginId());
      report.setUpdatedDatetime(shippingHeader.getUpdatedDatetime());
      report.setShippingNo(shippingNo);
      report.setOrderUpdatedDatetime(orderUpdatedDatetime);

      serviceResult = service.clearShippingReport(report);
  ...
  public String getActionName() {
    return Messages.getString("web.action.back.order.ShippingListClearShippingReportAction.4");
  }
