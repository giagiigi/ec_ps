 ...
 */
public class FreePostageEditRegisterAction extends FreePostageEditBaseAction {
  ...
  public boolean validate() {
    ...
    // 验证是否存在
    if (existFreePostage(bean.getFreePostageCode())) {
  ...
  public WebActionResult callService() {
    ...
    ServiceResult serviceResult = communicationService.insertFreePostageRule(freePostageRule);
    ...
    if (serviceResult.hasError()) {
      ...
      addErrorMessage(WebMessage.get(DataIOErrorMessage.CONTENTS_REGISTER_FAILED, bean.getFreePostageCode()));
      setRequestBean(getBean());
      setBean(getBean());
    ...
    // 添加成功
    setNextUrl("/app/communication/free_postage_edit/select/" + super.REGISTER + "/" + bean.getFreePostageCode());
    setRequestBean(getBean());
    setBean(getBean());
  ...
  public String getActionName() {
    return Messages.getString("web.action.back.communication.FreePostageEditRegisterAction.0");
  }
