 ...
 * @author OB.
 */
public class PublicCouponListMoveAction extends WebBackAction<PublicCouponListBean> {
  ...
  public WebActionResult callService() {
    ...
    if (UPDATE.equals(urlParam[0])) { 
      ...
      if (newCouponRule == null) {
        addErrorMessage(WebMessage.get(ServiceErrorMessage.NO_DATA_ERROR, "公共优惠券发行规则"));
    ...
    if (urlParam[0].equals("new")) {
      ...
      setNextUrl("/app/communication/public_coupon_edit/init/" + urlParam[1]);
    } else {
      addErrorMessage(WebMessage.get(CommunicationErrorMessage.MOVE_ACTION_ERROR, "公共优惠券规则编号"));
      setRequestBean(getBean());
      setNextUrl(null);
      return BackActionResult.RESULT_SUCCESS;
    }
    ...
    // 前画面情報設定
    DisplayTransition.add(getBean(), "/app/communication/public_coupon_list/search_back", getSessionContainer());
    setRequestBean(getBean());
    return BackActionResult.RESULT_SUCCESS;
  }
