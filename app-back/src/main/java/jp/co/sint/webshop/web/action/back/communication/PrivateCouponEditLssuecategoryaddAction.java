 ...
 * @author OB.
 */
public class PrivateCouponEditLssuecategoryaddAction extends PrivateCouponEditBaseAction {
  ...
  public WebActionResult callService() {
    ...
    String categoryCode = bean.getLssueCategoryCode();
    ...
    newCouponRuleLssueInfo.setCategoryCode(categoryCode);
    ...
    // 登录顾客别优惠券发行规则
    serviceResult = svc.insertNewCouponRuleLssue(newCouponRuleLssueInfo, WebConstantCode.LSSUE_CATEGORY_COMPLETE_REGISTER);
    ...
    if (serviceResult.hasError()) {
      for (ServiceErrorContent error : serviceResult.getServiceErrorList()) {
        if (error.equals(CommonServiceErrorContent.VALIDATION_ERROR)) {
          ...
          // 数据重复错误信息
          addErrorMessage(WebMessage.get(ServiceErrorMessage.REGISTER_ERROR, MessageFormat.format("发行关联分类编号：{0}", categoryCode)));
          setRequestBean(bean);
          return BackActionResult.RESULT_SUCCESS;
        }
      }
    ...
    setNextUrl("/app/communication/private_coupon_edit/init/edit/" + bean.getCouponCode() + "/"
        + WebConstantCode.LSSUE_CATEGORY_COMPLETE_REGISTER);
  ...
  public boolean validate() {
    PrivateCouponEditBean bean = getBean();
    if (StringUtil.isNullOrEmpty(bean.getLssueCategoryCode()) || bean.getLssueCategoryCode().equals("0")) {
      List<String> messages = new ArrayList<String>();
      messages.add("发行关联分类编号必须输入！");
