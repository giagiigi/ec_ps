 ...
 * @author KS.
 */
public class OptionalCampaignListDeleteAction extends OptionalCampaignListSearchAction {
  ...
  public WebActionResult callService() {
    ...
    OptionalCampaignListBean nextBean = getBean();

    // 選択されたレビューIDを取得する
    String[] values = getRequestParameter().getAll("campaignCode");
    ...
    for (String campaignCode : values) {
      ServiceResult serviceResult = service.deleteOptionalCampaign(getLoginInfo().getShopCode(),campaignCode);
      // 错误处理
      if (serviceResult.hasError()) {
        ...
        successList.add(WebMessage.get(CompleteMessage.DELETE_COMPLETE, "购买多件优惠活动(活动编号：" + campaignCode + ")"));
      }
    }
