 ...
 */
public class ShippingListSendShippingSmsAction extends ShippingListBaseAction {
  ...
  public WebActionResult callService() {
    ...
    SmsingService smsService = ServiceLocator.getSmsingService(getLoginInfo());
    ...
    for (String shippingNo : reqBean.getShippingCheck()) {
      ...
      if (NumUtil.toString(shippingHeader.getShippingStatus()).equals(ShippingStatus.CANCELLED.getValue())) {
        ...
        addErrorMessage(WebMessage.get(OrderErrorMessage.CANCELLED_SHIPPING_WITH_NO, shippingHeader.getShippingNo(),
            Messages.getString("web.action.back.order.ShippingListSendShippingSmsAction.0")));
        continue;
      }
      ...
      if (DataTransportStatus.fromValue(container.getOrderHeader().getDataTransportStatus())
        ...
        addErrorMessage(WebMessage.get(OrderErrorMessage.SEND_SHIPPING_SMS_DATA_TRANSPORTED_WITH_NO, shippingNo));
        continue;
      }
      ...
      //发送sms
      ServiceResult serviceResult = smsService.sendShippingReceivedSms(container,
          allContainer,
          shop,
          payment,
          shippingNo);
      ...
      if (serviceResult.hasError()) {
        for (ServiceErrorContent errorContent : serviceResult.getServiceErrorList()) {
          if (errorContent.equals(CommonServiceErrorContent.DB_OBJECT_EXECUTE_ERROR)) {
            addErrorMessage(WebMessage.get(ActionErrorMessage.REGISTER_SMSQUEUE_FAILED,
                Messages.getString("web.action.back.order.ShippingListSendShippingSmsAction.1") + shippingNo));
          } else if (errorContent.equals(SmsingServiceErrorContent.NO_SMS_TEMPLATE_ERROR)) {
            return BackActionResult.SERVICE_ERROR;
          } else if (errorContent.equals(SmsingServiceErrorContent.NO_CUSTOMER_TO_SEND_SMS)) {
            addErrorMessage(WebMessage.get(OrderErrorMessage.NO_CUSTOMER_TO_SEND_SHIPPING_SMS_WITH_NO, shippingNo));
          } else {
            addErrorMessage(WebMessage.get(OrderErrorMessage.SEND_SHIPPING_SMS_FAILED_WITH_NO, shippingNo));
          }
        }
      } else {
        addInformationMessage(WebMessage.get(CompleteMessage.SEND_SHIPPING_SMS_COMPLETE, shippingNo));
      }
    }
  ...
  public boolean validate() {
    ...
    if (reqBean.getShippingCheck().isEmpty()) {
      addErrorMessage(WebMessage.get(ActionErrorMessage.NO_CHECKED,
          Messages.getString("web.action.back.order.ShippingListSendShippingSmsAction.2")));
      result = false;
    }
  ...
  public String getActionName() {
    return Messages.getString("web.action.back.order.ShippingListSendShippingSmsAction.3");
  }
