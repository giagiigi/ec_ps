 ...
 */
public class ShippingListRegisterShippingDirectAction extends ShippingListBaseAction {
  ...
  public boolean validate() {
    ...
    if (reqBean.getShippingCheck().isEmpty()) {
      addErrorMessage(WebMessage.get(ActionErrorMessage.NO_CHECKED,
          Messages.getString("web.action.back.order.ShippingListRegisterShippingDirectAction.0")));
      result = false;
    }
  ...
  public WebActionResult callService() {
    ...
    for (String shippingNo : reqBean.getShippingCheck()) {
      ...
      if (shippingHeader == null) {
        errorMessageDetailList.add(WebMessage.get(ServiceErrorMessage.NO_DATA_ERROR,
            MessageFormat.format(Messages.getString(
                "web.action.back.order.ShippingListRegisterShippingDirectAction.1"), shippingNo)));
        continue;
      }
      ...
      // 出荷可能でない場合エラーとする
      if (shippingHeader.getShippingStatus().equals(ShippingStatus.READY.longValue())) {
        ...
        if (orderHeader == null) {
          errorMessageDetailList.add(WebMessage.get(ServiceErrorMessage.NO_DATA_ERROR,
              Messages.getString("web.action.back.order.ShippingListRegisterShippingDirectAction.3"),
        ...
        if (shippingHeader.getShippingDirectDate() == null) {
          // 出荷指示日が空の場合はエラー
          errorMessageDetailList.add(WebMessage.get(OrderErrorMessage.IMPOSSIBILITY_SHIPPING_DIRECT_CAUSE_DATE, shippingHeader
              .getShippingNo()));
          continue;
        }

        serviceResult = service.registerShippingDirection(shippedResult);
        if (serviceResult.hasError()) {
          for (ServiceErrorContent errorContent : serviceResult.getServiceErrorList()) {
            if (errorContent.equals(CommonServiceErrorContent.NO_DATA_ERROR)) {
              errorMessageDetailList.add(WebMessage.get(ServiceErrorMessage.NO_DATA_ERROR,
                  Messages.getString("web.action.back.order.ShippingListRegisterShippingDirectAction.1"),
        ...
        // 「出荷可能」以外の場合出荷指示不可
        errorMessageDetailList.add(WebMessage.get(OrderErrorMessage.IMPOSSIBILITY_SHIPPING_DIRECT, shippingHeader.getShippingNo()));
      }
    }
  ...
  public String getActionName() {
    return Messages.getString("web.action.back.order.ShippingListRegisterShippingDirectAction.4");
  }
