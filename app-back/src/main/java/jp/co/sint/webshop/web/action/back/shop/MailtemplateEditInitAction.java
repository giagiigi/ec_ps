 ...
 */
public class MailtemplateEditInitAction extends WebBackAction<MailtemplateEditBean> {
  ...
  public WebActionResult callService() {
    ...
    List<CodeAttribute> mailTypeList = new ArrayList<CodeAttribute>();
    mailTypeList.add(new NameValue(Messages.getString(
      "web.action.back.shop.MailtemplateEditInitAction.0"), ""));
    ...
    for (MailType type : MailTemplateUtil.getUsableMailTypeList(getConfig().getOperatingMode(), getLoginInfo().isSite(), false)) {
      if (type == MailType.INFORMATION || (notUsePoint && type == MailType.POINT_EXPIRED)) {
        continue;
      }
      mailTypeList.add(type);
    }
    bean.setMailTypeList(mailTypeList);

    List<CodeAttribute> informationMailList = new ArrayList<CodeAttribute>();
    informationMailList.add(new NameValue(Messages.getString(
      "web.action.back.shop.MailtemplateEditInitAction.0"), ""));
    ShopManagementService service = ServiceLocator.getShopManagementService(getLoginInfo());
    List<CodeAttribute> informationMailListDto = service.getInformationMailTypeList();
    informationMailList.addAll(informationMailListDto);
    informationMailList.add(new NameValue(Messages.getString(
      "web.action.back.shop.MailtemplateEditInitAction.1"), "new_template"));
    bean.setInformationMailList(informationMailList);

    List<CodeAttribute> contentTypeList = new ArrayList<CodeAttribute>();
    contentTypeList.addAll(Arrays.asList(MailContentType.values()));
    bean.setMailContentType(contentTypeList);
    bean.setContentType(MailContentType.TEXT.getValue());

    bean.setModeDiv("init");
    bean.setMailType("");
    bean.setNewMailTypeName("");
    bean.setTemplateDiv(StringUtil.coalesceEmptyValue(bean.getTemplateDiv(), "0"));
    setRequestBean(bean);
