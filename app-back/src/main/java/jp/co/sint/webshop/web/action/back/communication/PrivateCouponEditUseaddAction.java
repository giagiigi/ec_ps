 ...
 * @author OB.
 */
public class PrivateCouponEditUseaddAction extends PrivateCouponEditBaseAction {
  ...
  public WebActionResult callService() {
    ...
    String[] codeStringArray = bean.getUseCommodityCode().split("\r\n");
      ...
      for (int i = 0; i < codeStringArray.length; i++) {
        if (StringUtil.hasValue(codeStringArray[i])) {
          ...
          List<NewCouponRuleUseInfo> rule = svc.getPrivateCouponUse(bean.getCouponCode(), codeStringArray[i],
              WebConstantCode.USE_COMPLETE_REGISTER);
          if (rule != null) {
          ...
          newCouponRuleUseInfo.setCommodityCode(codeStringArray[i]);
