 ...
 * @author 
 */
public class DeliveryJdCompanyDateDeleteAction extends DeliveryJdCompanyDateBaseAction {
  ...
  public WebActionResult callService() {
		DeliveryJdCompanyDateBean bean=getBean();
		String[] params=getRequestParameter().getPathArgs();
		ShopManagementService service = ServiceLocator.getShopManagementService(getLoginInfo());
      
    JdDeliveryRelatedInfo info = service.getDeliveryRelatedInfoJd(params[0]);
    if (info ==null) {
      ...
      if (!bean.getPrefectureCode().equals(info.getPrefectureCode()) || !bean.getDeliveryCompanyNo().equals(info.getDeliveryCompanyNo())) {
        ...
        //执行删除功能
        ServiceResult serviceResult=service.deleteDeliveryRelatedInfoJd(info);
    ...
    List<JdDeliveryRelatedInfo> result=service.getDeliveryRelatedInfoListJd(getLoginInfo().getShopCode(), 
