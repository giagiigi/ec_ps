 ...
 */
public class ShippingDetailClearShippingDirectDateAction extends WebBackAction<ShippingDetailBean> {
  ...
  public WebActionResult callService() {
    ...
    if (shippingHeader == null) {
      addErrorMessage(WebMessage.get(ServiceErrorMessage.NO_DATA_ERROR, MessageFormat.format(
          Messages.getString("web.action.back.order.ShippingDetailClearShippingDirectDateAction.0"), shippingNo)));
    }
    ...
    if (NumUtil.toString(shippingHeader.getShippingStatus()).equals(ShippingStatus.CANCELLED.getValue())) {
      ...
      addErrorMessage(WebMessage.get(OrderErrorMessage.CANCELLED_SHIPPING_WITH_NO, shippingHeader.getShippingNo(),
          Messages.getString("web.action.back.order.ShippingDetailClearShippingDirectDateAction.1")));
    }
    ...
    if (orderHeader == null) {
      addErrorMessage(WebMessage.get(ServiceErrorMessage.NO_DATA_ERROR, MessageFormat.format(
          Messages.getString("web.action.back.order.ShippingDetailClearShippingDirectDateAction.2"),
    ...
    if (shippingHeader.getShippingStatus().equals(ShippingStatus.SHIPPED.longValue())) {
      addErrorMessage(WebMessage.get(OrderErrorMessage.IMPOSSIBILITY_CLEAR_SHIPPING_DIRECT_DATE, shippingHeader.getShippingNo()));
    }
    ...
    if (shippingHeader.getShippingStatus().equals(ShippingStatus.IN_PROCESSING.longValue())) {
      serviceResult = service.clearShippingDirectDate(shippedResult);
    } else {
      ...
      if (serviceResult.hasError()) {
        for (ServiceErrorContent errorContent : serviceResult.getServiceErrorList()) {
          if (errorContent.equals(CommonServiceErrorContent.NO_DATA_ERROR)) {
            addErrorMessage(WebMessage.get(ServiceErrorMessage.NO_DATA_ERROR, MessageFormat.format(
                Messages.getString("web.action.back.order.ShippingDetailClearShippingDirectDateAction.0"),
  ...
  public String getActionName() {
    return Messages.getString("web.action.back.order.ShippingDetailClearShippingDirectDateAction.3");
  }
