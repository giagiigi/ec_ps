 ...
 */
public class TmallCommodityListMoveAction extends WebBackAction<TmallCommodityListBean> {
  ...
  private static final String PICTURE_DESCRIBE = "describe";
  
  //2014/4/28 京东WBS对应 ob_李 add start
  private static final String JD_PICTURE_DESCRIBE = "jd_describe";
  ...
  public WebActionResult callService() {
    ...
    TmallCommodityListBean bean = getBean();
    ...
    if(shopCode==null||commodityCode==null){
    ...
    //add by os014 begin 2012-01-09
    if("edit".equals(nextPicture)&&shopCode!=null&&commodityCode!=null){
      nextUrl = nextUrl + "/tmall_commodity_edit/select/" + shopCode + "/" + commodityCode+"/"+nextPicture;
      setBean(bean);
      setNextUrl(nextUrl);
      DisplayTransition.add(getBean(), "/app/catalog/tmall_commodity_list/search_back", getSessionContainer());
      return BackActionResult.RESULT_SUCCESS; 
    }else if (PICTURE_CATEGORY.equals(nextPicture)) {
      nextUrl = nextUrl + "/tmall_related_category_tree/init/" + shopCode + "/" + commodityCode;
    }
    //add by os014 end 2012-01-09
    

    // 商品存在チェック
    
    // add by os012 20111228 end  
    ...
    if (nextPicture.equals(PICTURE_EDIT) || nextPicture.equals(PICTURE_COPY)) {
      ...
      nextUrl = nextUrl + "/tmall_related_category_tree/init/" + shopCode + "/" + commodityCode;
    } else if (PICTURE_DESCRIBE.equals(nextPicture)){
      nextUrl = nextUrl + "/tmall_related_describe/init/" + shopCode + "/" + commodityCode;
    } 
    //2014/4/28 京东WBS对应 ob_李 add start
    else if (JD_PICTURE_DESCRIBE.equals(nextPicture)){
      nextUrl = nextUrl + "/jd_property/init/" + shopCode + "/" + commodityCode;
    }
    //2014/4/28 京东WBS对应 ob_李 add end
    else {
