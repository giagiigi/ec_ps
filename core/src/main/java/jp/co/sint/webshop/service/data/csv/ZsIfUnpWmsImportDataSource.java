 ...
 * @author OS011
 */
public class ZsIfUnpWmsImportDataSource extends
    SqlImportDataSource<ZsIfUnpWmsImportCsvSchema, ZsIfUnpWmsImportCondition> {
  ...
  // 2014/06/12 库存更新对应 ob_张震 delete start
  // private PreparedStatement stockTempDelStatement = null;
  // 2014/06/12 库存更新对应 ob_张震 delete end
  ...
  // 2014/06/12 库存更新对应 ob_张震 update start
  // 添加库存临时表
  ...
  // 2014/06/12 库存更新对应 ob_张震 update end

  // 2014/06/12 库存更新对应 ob_张震 add start
  // 检验库存临时表
  ...
  // 2014/06/12 库存更新对应 ob_张震 add end
    ...
    if (zsIfInvImportReport == null) {
      summary.getErrors().add(
          new ValidationResult(null, null, Message.get(CsvMessage.WRONG_VALUE, Messages
              .getString("service.data.csv.ZsIfUnpImportDataSource.5"))));
    }
  ...
  public void executeUpdate(List<String> csvLine) throws SQLException {
    ...
    for (ZsIfInvImportReport zsIfReportList : importList) {
      ...
      try {
        ...
        // memo
        stockIoDeatilParams.add("(WMS-EC)盘点库存导入。" + "导入库存数：" + addStock);
