...
public interface CampaignDao extends GenericDao<Campaign, Long> {
   ...
   */
  Campaign load(String shopCode, String campaignCode);
   ...
   */
  Long insert(Campaign obj, LoginInfo loginInfo);
   ...
   * @param loginInfo ログイン情報
   */
  void update(Campaign obj, LoginInfo loginInfo);
   ...
   * @param loginInfo ログイン情報
   */
  void delete(Campaign obj, LoginInfo loginInfo);
   ...
   */
  List<Campaign> findByQuery(Query query);
   ...
   * @return 検索結果に相当するCampaignのリスト
   */
  List<Campaign> findByQuery(String sqlString, Object... params);

  /**
   * テーブルの全データを一括で取得します。
   * @return テーブルの全データ分のCampaignのリスト
   */
  List<Campaign> loadAll();
