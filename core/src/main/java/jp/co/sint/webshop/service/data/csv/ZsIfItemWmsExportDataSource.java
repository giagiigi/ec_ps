...
public class ZsIfItemWmsExportDataSource extends SqlExportDataSource<ZsIfItemWmsExportCsvSchema, ZsIfItemWmsExportCondition> {

  WebshopConfig config = DIContainer.getWebshopConfig();

  private boolean headerOnly = false;

  // 导出过程中临时设定的exportFlagWms值
  private Long exportFlagWms = Long.parseLong("9");
  ...
  String updateSql = "UPDATE C_COMMODITY_HEADER SET EXPORT_FLAG_WMS = ?, UPDATED_USER = ?, UPDATED_DATETIME = ?  WHERE EXPORT_FLAG_WMS = ? AND (SET_COMMODITY_FLG <> 1 OR SET_COMMODITY_FLG IS NULL) AND COMMODITY_CODE <> '9999999' AND COMMODITY_NAME NOT LIKE '%已删除%'";

  /*
   * 导出前把数据库中所有的EXPORT_FLAG_WMS值为0的数据的EXPORT_FLAG_WMS值更新成9
   */
  public void beforeExport() {
    ...
    updateShippingParams.add(getExportFlagWms());
  ...
  /*
   * 导出成功后把数据库中所有的EXPORT_FLAG_WMS值为9的数据的EXPORT_FLAG_WMS值更新成1
   */
...
//        + " ,(SELECT PATH||'~'||CATEGORY_CODE FROM CATEGORY WHERE CATEGORY_CODE = "
//        + "  (SELECT MIN(CC.CATEGORY_CODE) FROM CATEGORY_COMMODITY CC "
//        + "  INNER JOIN CATEGORY C ON C.CATEGORY_CODE = CC.CATEGORY_CODE "
//        + "  WHERE CC.COMMODITY_CODE=A.COMMODITY_CODE AND CC.CATEGORY_CODE != '0')) AS PRODUCTLINE"
        ...
        + " , A.IN_BOUND_LIFE_DAYS"+",A.OUT_BOUND_LIFE_DAYS"+",A.SHELF_LIFE_DAYS"
        + " ,A.SHELF_LIFE_ALERT_DAYS"+"  FROM C_COMMODITY_HEADER A "
        ...
        + " WHERE (A.SET_COMMODITY_FLG <> 1 OR A.SET_COMMODITY_FLG IS NULL) AND A.COMMODITY_NAME NOT LIKE '%已删除%' AND A.COMMODITY_CODE <> '9999999'  AND (A.ORIGINAL_COMMODITY_CODE IS NULL OR A.ORIGINAL_COMMODITY_CODE = '') AND A.EXPORT_FLAG_WMS =" + getExportFlagWms();
