...
public class NearCommodityWarnDataSource extends SqlExportDataSource<NearCommodityWarnCsvSchema, NearCommodityWarnCondition> {

  public Query getExportQuery() {
    String sql = "SELECT ('''' || COMMODITY_CODE) AS COMMODITY_CODE, COMMODITY_NAME FROM COMMODITY_HEADER WHERE SHELF_LIFE_DAYS IS NOT NULL ORDER BY COMMODITY_CODE";
    ...
    for (int i = 0; i < row.size(); i++) {
      ...
      if (columnName.equals("COMMODITY_CODE") || columnName.equals("commodity_code")) {
        ...
        if(flg) {
          ...
          CommodityPreviousDay commodityPreviousDay = new CommodityPreviousDay();
          commodityPreviousDay.setCommodityCode(commodityCode.replace("'", ""));
          
          commodityPreviousDayDao.insert(commodityPreviousDay);
        }
        break;
      }
    }
