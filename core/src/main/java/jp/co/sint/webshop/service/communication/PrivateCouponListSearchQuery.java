...
public class PrivateCouponListSearchQuery extends AbstractQuery<NewCouponRule> {
  ...
  public PrivateCouponListSearchQuery(PrivateCouponListSearchCondition condition, boolean flag) {
	  buildQuery(BASE_QUERY, condition, flag);
  }
  //2013/04/12 优惠券对应 ob modify start
  //public static final String FIND_NEW_COUPON_USE_START_FOR_BATCH = "SELECT * FROM NEW_COUPON_HISTORY WHERE " +
  //  " ? >= USE_START_DATETIME AND ? <= USE_START_DATETIME";
  
  //public static final String FIND_NEW_COUPON_USE_END_FOR_BATCH = "SELECT * FROM NEW_COUPON_HISTORY WHERE " +
	//" ? >= USE_END_DATETIME AND ? <= USE_END_DATETIME AND USE_STATUS = " + UseStatus.UNUSED.getValue();
  
  //优惠劵利用开始通知
  public static final String FIND_NEW_COUPON_USE_START_FOR_BATCH = "SELECT * FROM NEW_COUPON_HISTORY WHERE "
      + " TO_CHAR(USE_START_DATETIME, 'yyyy/MM/dd') = ? "
      + " AND USE_STATUS = " + UseStatus.UNUSED.getValue()
      + " AND COUPON_STATUS = " + CouponStatus.TEMPORARY.getValue();

  //优惠劵利用结束通知
  public static final String FIND_NEW_COUPON_USE_END_FOR_BATCH = "SELECT * FROM NEW_COUPON_HISTORY WHERE "
      + " TO_CHAR(USE_END_DATETIME, 'yyyy/MM/dd') = ? "
      + " AND USE_STATUS = " + UseStatus.UNUSED.getValue()
      + " AND COUPON_STATUS = " + CouponStatus.USED.getValue();
  // 2013/04/12 优惠券对应 ob modify end
  
  //当优惠券的发行最小购买金额相等时判断优惠券利用其间是否重复
  public static final String CHECK_DUPLICATED_REGISTER = "SELECT COUNT(COUPON_CODE) FROM NEW_COUPON_RULE " 
  ...
  //判断优惠券信息是否已经被使用
  public static final String CHECK_NEWCOUPONRULE_USE = "SELECT COUNT(COUPON_CODE) FROM NEW_COUPON_HISTORY WHERE COUPON_CODE = ? ";
  
  private static final String BASE_QUERY = "SELECT NCR.COUPON_CODE,"
	+ " NCR.COUPON_NAME, "
	+ " NCR.COUPON_NAME_EN, "
	+ " NCR.COUPON_NAME_JP, "
	+ " NCR.COUPON_TYPE, "
  ...
  private void buildQuery(String query, PrivateCouponListSearchCondition condition, boolean flag) {
    StringBuilder builder = new StringBuilder(query);
    //根据查询条件取得顾客别优惠券发行规则信息
    if (flag) {
		builder.append(" WHERE NCR.COUPON_TYPE != " + CouponType.COMMON_DISTRIBUTION.longValue());
	//根据查询条件取得顾客组别优惠规则信息
    } else {
    ...
    if (StringUtil.hasValue(condition.getSearchCouponNameEn())) {
        SqlDialect dialect = SqlDialect.getDefault();
        SqlFragment fragment = dialect.createLikeClause("NCR.COUPON_NAME_EN", condition
    ...
    if (StringUtil.hasValue(condition.getSearchCouponNameJp())) {
        SqlDialect dialect = SqlDialect.getDefault();
        SqlFragment fragment = dialect.createLikeClause("NCR.COUPON_NAME_JP", condition
    ...
    if (StringUtil.hasValueAnyOf(condition.getSearchMinIssueStartDatetimeFrom(), condition.getSearchMinIssueStartDatetimeTo())) {
      ...
      SqlFragment fragment = dialect.createRangeClause("NCR.MIN_ISSUE_START_DATETIME", DateUtil.fromString(condition.getSearchMinIssueStartDatetimeFrom()
          , true), DateUtil.fromString(endDate, true));
      builder.append(" AND ");
      builder.append(fragment.getFragment());
      params.addAll(Arrays.asList(fragment.getParameters()));
     /* builder.append(" AND NCR.COUPON_TYPE = ? OR (NCR.COUPON_TYPE = ? ))");
      params.add(CouponType.PURCHASE_DISTRIBUTION.getValue());
      params.add(CouponType.SPECIAL_MEMBER_DISTRIBUTION.getValue());*/
    }
    // 检索条件：优惠券发行结束日时
    if (StringUtil.hasValueAnyOf(condition.getSearchMinIssueEndDatetimeFrom(), condition.getSearchMinIssueEndDatetimeTo())) {
      SqlDialect dialect = SqlDialect.getDefault();
      ...
      SqlFragment fragment = dialect.createRangeClause("NCR.MIN_ISSUE_END_DATETIME", DateUtil.fromString(condition.getSearchMinIssueEndDatetimeFrom()
          , true), DateUtil.fromString(endDate, true));
      builder.append(" AND ");
      builder.append(fragment.getFragment());
      params.addAll(Arrays.asList(fragment.getParameters()));
    ...
    if (StringUtil.hasValue(condition.getSearchCouponActivityStatus())) {
		...
		if (ActivityStatus.IN_PROGRESS.getValue().equals(condition.getSearchCouponActivityStatus())) {
			builder.append(" AND NCR.MIN_ISSUE_START_DATETIME <= " + SqlDialect.getDefault().getCurrentDatetime() + " AND NCR.MIN_ISSUE_END_DATETIME >= " + SqlDialect.getDefault().getCurrentDatetime());
    		
    	//未发行
    	}else if(ActivityStatus.NOT_PROGRESS.getValue().equals(condition.getSearchCouponActivityStatus())){
    		builder.append(" AND NCR.MIN_ISSUE_START_DATETIME > " + SqlDialect.getDefault().getCurrentDatetime());
    		
    	//已发行
    	}else if(ActivityStatus.OUT_PROGRESS.getValue().equals(condition.getSearchCouponActivityStatus())){
    		builder.append(" AND NCR.MIN_ISSUE_END_DATETIME < " + SqlDialect.getDefault().getCurrentDatetime());
    	}
    }

    //排序
    builder.append(" ORDER BY CASE"
    + " WHEN NCR.MIN_ISSUE_START_DATETIME <= " + SqlDialect.getDefault().getCurrentDatetime()
    + " AND NCR.MIN_ISSUE_END_DATETIME >= " + SqlDialect.getDefault().getCurrentDatetime() + " THEN 1 "
    + " WHEN NCR.MIN_ISSUE_START_DATETIME > " + SqlDialect.getDefault().getCurrentDatetime() + " THEN 2 "
    + " ELSE 3 END,"
    + " NCR.MIN_ISSUE_START_DATETIME DESC");
  ...
  public Class<NewCouponRule> getRowType() {
    return NewCouponRule.class;
  }
