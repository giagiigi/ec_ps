 ...
 *
 */
public interface JdOrderHeaderDao extends GenericDao<JdOrderHeader, Long> {
   ...
   */
  JdOrderHeader loadByRowid(Long id);
   ...
   */
  JdOrderHeader load(String orderNo);
  /**
   * 主キー列の値を指定して受注ヘッダのインスタンスを取得します。
   *
   * @param TmallId 受注番号
   ...
   */
  JdOrderHeader loadByTid(String tid);
  /**
   ...
   */
  Long insert(JdOrderHeader obj, LoginInfo loginInfo);
   ...
   * @param loginInfo ログイン情報
   */
  void update(JdOrderHeader obj, LoginInfo loginInfo);
   ...
   * @param loginInfo ログイン情報
   */
  void delete(JdOrderHeader obj, LoginInfo loginInfo);
   ...
   */
  List<JdOrderHeader> findByQuery(Query query);
   ...
   * @return 検索結果に相当するOrderHeaderのリスト
   */
  List<JdOrderHeader> findByQuery(String sqlString, Object... params);

  /**
   * テーブルの全データを一括で取得します。
   * @return テーブルの全データ分のOrderHeaderのリスト
   */
  List<JdOrderHeader> loadAll();
