...
public class AttributeFrontQuery extends AbstractQuery<CommodityKey> {
  ...
  private StringBuilder getInitializedBuffer() {
    ...
    // select clause
    b.append("SELECT SUBSTRING ( CATEGORY_ATTRIBUTE1 , 0, POSITION ( '|' IN CATEGORY_ATTRIBUTE1 ) ) CATEGORY_ATTRIBUTE, SUBSTRING(CATEGORY_ATTRIBUTE1, POSITION('|' IN CATEGORY_ATTRIBUTE1) + 1) CATEGORY_ATTRIBUTE1, SUM(CH.COMMODITY_POPULAR_RANK) COMMODITY_POPULAR_RANK, COUNT ( COMMODITY_CODE ) COMMODITY_COUNT ");
    ...
    // postgreSQL end
    b.append("AND CH.SALE_FLG = 1 AND REPRESENT_SKU_CODE = SKU_CODE AND CATEGORY_ATTRIBUTE1 IS NOT NULL ");
  ...
  public AttributeFrontQuery(CommodityContainerCondition condition) {
...
//      b.append("WHERE CC.SHOP_CODE = CH.SHOP_CODE AND CC.COMMODITY_CODE = CH.COMMODITY_CODE AND ");
    ...
    // 並び順設定
    b.append(" GROUP BY SUBSTRING ( CATEGORY_ATTRIBUTE1 , 0, POSITION ( '|' IN CATEGORY_ATTRIBUTE1 ) ), SUBSTRING(CATEGORY_ATTRIBUTE1, POSITION('|' IN CATEGORY_ATTRIBUTE1) + 1)  ORDER BY CATEGORY_ATTRIBUTE1 ");
  ...
  private void setKeyWordCondition(CommodityContainerCondition condition, StringBuilder builder, List<Object> params) {
    ...
    if (!searchWordList.isEmpty()) {
      ...
      for (int i = 0; i < searchWordList.size(); i++) {
        ...
        builder.append(" OR (SELECT CATEGORY_NAME_PC FROM CATEGORY WHERE PATH || '~' || CATEGORY_CODE = CH.CATEGORY_PATH) LIKE ? ");
        params.add("%" + SqlDialect.getDefault().escape(searchWordList.get(i).trim()) + "%");
      }
      builder.append(")");
    }
