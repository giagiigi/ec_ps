...
public class ReviewSummaryFrontQuery extends AbstractQuery<CommodityKey> {
  ...
  public ReviewSummaryFrontQuery() {
  ...
  private StringBuilder getInitializedBuffer() {
    ...
    // select clause
    b.append("SELECT  CASE WHEN R.REVIEW_SCORE = 100 THEN 80 ELSE R.REVIEW_SCORE END , COUNT(R.COMMODITY_CODE) COMMODITY_COUNT ");

    // from clause and joinees
    b.append("FROM  REVIEW_SUMMARY R INNER JOIN COMMODITY_LIST_VIEW CH ON R.COMMODITY_CODE = CH.COMMODITY_CODE ");
  ...
  public ReviewSummaryFrontQuery(CommodityContainerCondition condition) {
    ...
    // 並び順設定
    b.append(" GROUP BY R.REVIEW_SCORE ORDER BY R.REVIEW_SCORE ");
