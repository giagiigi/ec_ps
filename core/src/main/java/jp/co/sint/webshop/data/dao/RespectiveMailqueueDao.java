...
/** 
 * 「個別配信メールキュー(RESPECTIVE_MAILQUEUE)」テーブルへのデータアクセスを担当するDAO(Data Access Object)です。
 *
 * @author System Integrator Corp.
 *
 */
public interface RespectiveMailqueueDao extends GenericDao<RespectiveMailqueue, Long> {
   ...
   */
  RespectiveMailqueue loadByRowid(Long id);
   ...
   */
  RespectiveMailqueue load(Long mailQueueId);
   ...
   */
  Long insert(RespectiveMailqueue obj, LoginInfo loginInfo);
   ...
   * @param loginInfo ログイン情報
   */
  void update(RespectiveMailqueue obj, LoginInfo loginInfo);
   ...
   * @param loginInfo ログイン情報
   */
  void delete(RespectiveMailqueue obj, LoginInfo loginInfo);
   ...
   */
  List<RespectiveMailqueue> findByQuery(Query query);
   ...
   * @return 検索結果に相当するRespectiveMailqueueのリスト
   */
  List<RespectiveMailqueue> findByQuery(String sqlString, Object... params);

  /**
   * テーブルの全データを一括で取得します。
   * @return テーブルの全データ分のRespectiveMailqueueのリスト
   */
  List<RespectiveMailqueue> loadAll();
