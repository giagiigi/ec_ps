...
public class GiftCardLogExportQuery extends SimpleQuery {

  /** serial version UID */
  private static final long serialVersionUID = 1L;

  private static final String ANALYSIS_QUERY = "SELECT GCID.CARD_CODE, GCID.CARD_NAME ,GCID.ISSUE_DATE AS ISSUE_TIME ,COUNT(GCID.CARD_ID) AS ISSUE_NUM ,'￥'||SUM (GCID.UNIT_PRICE)||'' AS TOTAL_SALE_PRICE ,'￥'||SUM (GCID.DENOMINATION)||'' AS TOTAL_DENOMINATION "
  ...
  public GiftCardLogExportQuery(GiftCardLogSearchCondition condition) {
    List<Object> params = new ArrayList<Object>();
    StringBuilder builder = new StringBuilder();

    builder.append(ANALYSIS_QUERY);
    builder.append(" 1 = 1 ");
    ...
    if (StringUtil.hasValue(condition.getSearchGiftCardName())) {
      SqlDialect dialect = SqlDialect.getDefault();
      SqlFragment fragment = dialect.createLikeClause("GCR.CARD_NAME", condition.getSearchGiftCardName(),
          LikeClauseOption.PARTIAL_MATCH);
      builder.append(" AND " + fragment.getFragment());
      params.addAll(Arrays.asList(fragment.getParameters()));
    }
    ...
    if (StringUtil.hasValueAnyOf(condition.getSearchMinIssueStartDatetimeFrom(), condition.getSearchMinIssueStartDatetimeTo())) {
      ...
      SqlFragment fragment = dialect.createRangeClause("GCIH.ISSUE_DATE", DateUtil.fromString(condition
          .getSearchMinIssueStartDatetimeFrom(), true), DateUtil.fromString(endDate, true));
      builder.append(" AND ");
